...
#pragma omp parallel
  {
  double* new_tmp = (double*)malloc(block * block * sizeof(double));
  int j, n_i, n_j, n_k, n_kk;
  /* block index */
  int k, h;
#else
  int j, n_i, n_j, n_k, n_kk;
...
#pragma omp for
  ...
  for(i = 0; i < size; i += block){
	...
	for(h = 0; h < block; h++){
#ifdef LINK_OMP
	  new_tmp[h*block + k] = mat[n_i + j + n_k + h];
#else
      ...
      for(k = 0; k < block; k++){
      	...
      	for(h = 0; h < block; h++){
#ifdef LINK_OMP
      	  transp[n_j + i + n_k + h] = new_tmp[n_kk + h];
#else
