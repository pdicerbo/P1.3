 ...
 *
 *    This code implement the version presented in class using a single temp buffer (temp_new no longer needed).
 *    Purpose of this code is to introduce data-dependency with the main eveolve loop 
 *    which exclude a stright usage of a work-sharing construct.
 *
...
#ifdef __OPENMP
#pragma omp parallel private( iy_tmp, iy, ix, th_Id, ix_start, ix_end ) firstprivate( alpha, dx, dy, dt, nx, ny )    
    {
