 ...
 *    Parallelization is applied only to the most computationally intensive loop
 *    within the evolve(...) function.
 *
...
void evolve(int nx, int ny, MYFLOAT lx, MYFLOAT ly, MYFLOAT dt, const MYFLOAT *temp, MYFLOAT *temp_new, MYFLOAT alpha){
...
#ifdef __OPENMP
    /* Parallelized section. When OpenMP is activated the loop over the iy index is splitted among threads.
     * Default work-load distribution (static) is used. 
     */
#pragma omp parallel for private( iy, ix ) firstprivate( alpha, dx, dy, dt, nx, ny )    
#endif
    for(iy=1;iy<=ny;++iy){
      for(ix=1;ix<=nx;++ix){
...
void swap_ptr( MYFLOAT ** temp, MYFLOAT ** temp_new){
